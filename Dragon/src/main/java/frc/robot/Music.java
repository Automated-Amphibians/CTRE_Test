...
public class music {
    ...
    // --- Main: demo/test with one motor ---
    public static void main(String[] args) throws Exception {
        Path songPath;
        if (args.length >= 1) {
            songPath = Paths.get(args[0]);
            if (!Files.exists(songPath)) {
                System.err.println("Provided song file does not exist: " + songPath);
                return;
            }
        } else {
            songPath = createSampleSongFile();
        }

        System.out.println("Loading song from: " + songPath);
        Song song = Song.loadFromFile(songPath);
        System.out.println("Loaded " + song.events.size() + " events; song length " + song.lengthMs() + " ms");

        Orchestra orchestra = new Orchestra();

        // For demo: ensure motor 1 exists (you can add motors programmatically instead)
        Motor demoMotor = orchestra.addMotor(1);
        System.out.println("Added demo motor with CAN id: " + demoMotor.getCanId());

        Player player = new Player(orchestra);
        CompletableFuture<Void> done = player.play(song);

        System.out.println("Playback started. Waiting for completion...");
        // block until done
        done.get();
        System.out.println("Playback complete. Exiting.");
    }
}
